...
async function bootstrap() {
  logger.log('Starting Content/Engagement Microservice...');
  ...
  if (process.env.NODE_ENV !== 'production') {
    const config = new DocumentBuilder()
      .setTitle('HOS Content Service')
      .setDescription('CMS, Promotions, Gamification, Social Sharing API')
      .setVersion('1.0.0')
  ...
  const port = process.env.CONTENT_PORT || process.env.PORT || 3013;
  await app.listen(port, '0.0.0.0');
  logger.log(`Content service running on http://0.0.0.0:${port}`);
}
