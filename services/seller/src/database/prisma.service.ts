...
import { PrismaClient } from '../../node_modules/.prisma/seller-client';

@Injectable()
export class SellerPrismaService extends PrismaClient implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    while (retries > 0) {
      try { await this.$connect(); this.logger.log('Connected to database'); return; }
        ...
        retries--;
        this.logger.warn(`DB connection failed (${5 - retries}/5): ${error?.message}`);
