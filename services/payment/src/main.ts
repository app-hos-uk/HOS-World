...
async function bootstrap() {
  logger.log('Starting Payment Microservice...');
  ...
  if (process.env.NODE_ENV !== 'production') {
    const config = new DocumentBuilder().setTitle('HOS Payment Service').setVersion('1.0.0').addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'JWT' }, 'JWT-auth').build();
  ...
  const port = process.env.PAYMENT_PORT || process.env.PORT || 3009;
  await app.listen(port, '0.0.0.0');
  logger.log(`Payment service running on http://0.0.0.0:${port}`);
}

bootstrap().catch((error) => { logger.error(`Payment service failed: ${error?.message}`); process.exit(1); });
