...
async function bootstrap() {
  logger.log('Starting Admin/Analytics Microservice...');
  ...
  if (process.env.NODE_ENV !== 'production') {
    const config = new DocumentBuilder()
      .setTitle('HOS Admin Service')
      .setDescription('Admin, Analytics, Support, GDPR, Webhooks, Integrations API')
      .setVersion('1.0.0')
  ...
  const port = process.env.ADMIN_PORT || process.env.PORT || 3014;
  await app.listen(port, '0.0.0.0');
  logger.log(`Admin service running on http://0.0.0.0:${port}`);
}
