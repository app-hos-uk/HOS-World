...
export class HealthController {
  ...
  @Get() async health() { const db = await this.prisma.healthCheck(); return { status: db ? 'ok' : 'degraded', service: 'influencer-service', uptime: Math.floor((Date.now() - this.startTime) / 1000) }; }
