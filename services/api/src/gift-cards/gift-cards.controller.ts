...
export class GiftCardsController {
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Filter by status: ACTIVE, REDEEMED, CANCELLED, PENDING',
  })
  @ApiQuery({
    name: 'type',
    required: false,
    type: String,
    description: 'Filter by type: digital, physical',
  })
  @SwaggerApiResponse({ status: 200, description: 'Gift cards retrieved successfully' })
  @SwaggerApiResponse({ status: 401, description: 'Unauthorized' })
  @SwaggerApiResponse({ status: 403, description: 'Forbidden - Admin access required' })
  async listAllGiftCards(
    ...
    const pageNum = Math.max(1, parseInt(page, 10) || 1);
    const limitNum = Math.min(100, Math.max(1, parseInt(limit, 10) || 20));
    const result = await this.giftCardsService.listAll(
      pageNum,
      limitNum,
      status || undefined,
      type || undefined,
