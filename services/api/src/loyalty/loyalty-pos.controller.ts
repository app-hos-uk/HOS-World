  ...
  async redeemForVoucher(
    ...
    this.activity
      .createLog({
        ...
        description: `Burned ${body.points} points for a POS ${(data as { type?: string })?.type === 'PROMO_CODE' ? 'promo code' : 'gift card'} voucher`,
        metadata: {
          actor: req?.user?.id ? 'staff' : 'api-key',
          role: req?.user?.role ?? null,
          storeId: body.storeId,
          points: body.points,
          type: (data as { type?: string })?.type ?? null,
          promoCode: (data as { promoCode?: string })?.promoCode ?? null,
