...
CMD ["sh", "-c", "echo '═══════════════════════════════════════════════════════════' && echo '🚀 STARTUP SCRIPT STARTED' && echo \"Timestamp: $(date)\" && echo \"Working directory: $(pwd)\" && echo \"Node version: $(node --version)\" && echo 'Files in current directory:' && ls -la | head -10 && echo '' && echo '🔄 STEP 1: Running database migrations...' && if pnpm exec prisma migrate deploy 2>&1; then echo '✅ Migrations complete.'; else echo '❌ CRITICAL: Database migrations failed!' && echo 'The API cannot start without successful migrations.' && exit 1; fi && echo '' && echo '🔄 STEP 2: Generating Prisma Client...' && if pnpm exec prisma generate 2>&1 | tee /tmp/prisma-gen.log; then echo '✅ Prisma Client generated.'; else echo '❌ CRITICAL: Prisma Client generation failed!' && exit 1; fi && echo '' && echo '🔄 STEP 3: Verifying RefreshToken model in generated client...' && if node -e \"try { const { PrismaClient } = require('@prisma/client'); const p = new PrismaClient(); const hasRefreshToken = typeof p.refreshToken !== 'undefined'; const hasUser = typeof p.user !== 'undefined'; console.log('  user model:', hasUser ? 'YES ✅' : 'NO ❌'); console.log('  refreshToken model:', hasRefreshToken ? 'YES ✅' : 'NO ❌'); if (!hasRefreshToken) { console.error('❌ CRITICAL: RefreshToken missing! Available:', Object.keys(p).filter(k => !k.startsWith('$') && !k.startsWith('_')).slice(0, 10).join(', ')); process.exit(1); } p.\$disconnect().catch(() => {}); } catch(e) { console.error('❌ Error:', e.message); process.exit(1); }\"; then echo '✅ Verification passed - RefreshToken found'; else echo '❌ CRITICAL: RefreshToken verification failed!' && exit 1; fi && echo '═══════════════════════════════════════════════════════════' && echo '🚀 Starting API...' && echo 'Checking dist/main.js exists:' && (test -f dist/main.js && echo '✅ dist/main.js found' || (echo '❌ dist/main.js NOT FOUND!' && ls -la dist/ && exit 1)) && echo 'Starting node...' && node dist/main.js"]
