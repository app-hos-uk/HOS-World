...
const SHIPPING_PROVIDERS: Record<string, ProviderMetadata> = {
  ...
  fedex: {
    displayName: 'FedEx',
    description: 'Global courier delivery services with real-time rates, labels, and tracking',
  ...
  dhl: {
    ...
    description: 'International express shipping with MyDHL API integration',
...
export default function ShippingIntegrationsPage() {
  ...
  const handleConfigure = (provider: string, integration?: Integration) => {
    const metadata = SHIPPING_PROVIDERS[provider];
      ...
      <AdminLayout>
        ...
        {loading ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {Object.entries(SHIPPING_PROVIDERS).map(([provider, metadata]) => {
