            ...
            <table className="w-full">
              <thead className="bg-hos-bg-secondary">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-hos-text-muted uppercase tracking-wider">Eligibility</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-hos-text-muted uppercase tracking-wider">Usage</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-hos-text-muted uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-hos-text-muted uppercase tracking-wider">Dates</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-hos-text-muted uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="bg-hos-bg-secondary divide-y divide-hos-border">
                {promotions.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-6 py-4 text-center text-hos-text-muted">
              ...
              <div className="p-6">
                ...
                <form onSubmit={handleCreatePromotion} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-hos-text-secondary border-b pb-1">Basics</h3>
                    <div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <label className="flex items-center ml-4">
                        <input
                          ...
                          className="mr-2"
                        />
                        <span className="text-sm text-hos-text-secondary">Stackable</span>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-hos-text-secondary border-b pb-1">Promotion Type & Value</h3>
                    <div>
                    ...
                    {formData.type === 'PERCENTAGE_DISCOUNT' && (
                      <div>
                        <label className="block text-sm font-medium text-hos-text-secondary mb-1">Percentage *</label>
                        <input
                          type="number"
                          step="0.01"
                          min={0}
                    ...
                    {formData.type === 'FIXED_DISCOUNT' && (
                      <div>
                        <label className="block text-sm font-medium text-hos-text-secondary mb-1">Fixed Amount *</label>
                        <input
                          type="number"
                          step="0.01"
                          min={0}
                    ...
                    {formData.type === 'BUY_X_GET_Y' && (
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-hos-text-secondary mb-1">Buy quantity (X) *</label>
                          <input
                            type="number"
                            min={1}
                        ...
                        <div>
                          <label className="block text-sm font-medium text-hos-text-secondary mb-1">Get quantity (Y) *</label>
                          <input
                            type="number"
                            min={1}
                  ...
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-hos-text-secondary border-b pb-1">Conditions</h3>
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center">
                        <input
                          ...
                          className="mr-2"
                        />
                        <span className="text-sm text-hos-text-secondary">Minimum order amount</span>
                      </label>
                      {formData.requirementType === 'MIN_ORDER_AMOUNT' && (
                        <input
                          ...
                          placeholder="Amount"
                          className="w-full max-w-xs px-4 py-2 border border-hos-border rounded-lg focus:outline-none focus:ring-2 focus:ring-hos-gold/50"
                        />
                      )}
                      <label className="flex items-center">
                        <input
                          ...
                          className="mr-2"
                        />
                        <span className="text-sm text-hos-text-secondary">Minimum quantity (items)</span>
                      </label>
                      ...
                      <label className="flex items-center">
                        <input
                          ...
                          className="mr-2"
                        />
                        <span className="text-sm text-hos-text-secondary">No minimum</span>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-hos-text-secondary border-b pb-1">Eligibility</h3>
                    <div className="flex flex-col gap-2">
                      <label className="flex items-center">
                        <input
                          type="radio"
                          ...
                          className="mr-2"
                        />
                        <span className="text-sm text-hos-text-secondary">Specific products</span>
                      </label>
                          ...
                          {productSearchResults.length > 0 && (
                            <ul className="border border-hos-border rounded-lg max-h-40 overflow-y-auto">
                              {productSearchResults.map((p) => (
                                <li key={p.id} className="flex items-center justify-between px-3 py-2 hover:bg-hos-bg-tertiary border-b border-hos-border last:border-0">
                                  <span className="text-sm truncate">{p.name ?? p.title}</span>
                                  <button
                                    type="button"
                                    onClick={() => addProduct(p)}
                                    disabled={formData.productIds.includes(p.id)}
                                    className="text-hos-gold text-sm font-medium disabled:opacity-50"
                                  >
                          ...
                          {selectedProducts.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-2">
                              {selectedProducts.map((sp) => (
                                ...
                                >
                                  {sp.name}
                                  <button type="button" onClick={() => removeProduct(sp.id)} className="ml-1 text-hos-gold hover:text-hos-gold-hover">&times;</button>
                                </span>
                              ))}
                            </div>
                          )}
                      ...
                      <label className="flex items-center">
                        <input
                          ...
                          className="mr-2"
                        />
                        <span className="text-sm text-hos-text-secondary">Specific categories / collections</span>
                      </label>
                          ...
                          {filteredCategories.length > 0 && (
                            <ul className="border border-hos-border rounded-lg max-h-40 overflow-y-auto">
                              {filteredCategories.map((c) => (
                                <li key={c.id} className="flex items-center justify-between px-3 py-2 hover:bg-hos-bg-tertiary border-b border-hos-border last:border-0">
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                              {formData.categoryIds.map((id) => {
                                ...
                                return (
                                  ...
                                  >
                                    {c?.name ?? id.slice(0, 8)}…
                                    <button type="button" onClick={() => removeCategory(id)} className="ml-1 text-hos-gold hover:text-hos-gold-hover">&times;</button>
                                  </span>
                                );
                              })}
                            </div>
                      ...
                      <label className="flex items-center">
                        <input
                          ...
                          className="mr-2"
                        />
                        <span className="text-sm text-hos-text-secondary">All products</span>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <h3 className="text-sm font-semibold text-hos-text-secondary border-b pb-1">Limitations / Usage</h3>
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        className="mr-2"
                      />
                      <span className="text-sm text-hos-text-secondary">Limit total uses</span>
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        className="mr-2"
                      />
                      <span className="text-sm text-hos-text-secondary">Limit to one use per customer</span>
