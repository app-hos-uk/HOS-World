...
export default function AdminLoyaltyMembersPage() {
  ...
  return (
    <RouteGuard allowedRoles={['ADMIN']} showAccessDenied>
      <AdminLayout>
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white">Loyalty Members</h1>
                ...
                {members.map((m) => (
                  <tr key={m.id || m.userId} className="hover:bg-hos-bg-tertiary">
                    <td className="px-4 py-3">
                      <div>
                        <p className="font-medium text-white">{m.user?.firstName} {m.user?.lastName}</p>
                    ...
                    <td className="px-4 py-3 text-right font-semibold text-white">{Number(m.pointsBalance ?? 0).toLocaleString()}</td>
