...
export default function FinanceRevenuePage() {
  ...
  const handleExport = async () => {
    try {
      setExporting(true);
      const response = await apiClient.exportTransactions(
        'csv',
        dateRange.startDate,
        dateRange.endDate
      );
  ...
  const totalRevenue =
    ...
    periods.reduce((sum: number, p: any) => sum + (p.totalRevenue || p.revenue || 0), 0);
  const totalOrders =
    reportData?.totalOrders ??
    periods.reduce((sum: number, p: any) => sum + (p.orderCount || p.orders || 0), 0);
  const avgOrderValue =
    reportData?.averageOrderValue ?? (totalOrders > 0 ? totalRevenue / totalOrders : 0);
    ...
    <RouteGuard allowedRoles={['FINANCE', 'ADMIN']} showAccessDenied={true}>
      ...
      >
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold">Revenue Reports</h1>
            <p className="text-hos-text-secondary mt-2">View revenue analytics and reports</p>
          </div>
          <button
          ...
          <>
            ...
            {periods.length === 0 ? (
              <div className="bg-hos-bg-secondary border border-hos-border rounded-lg p-12 text-center">
                ...
                <p className="text-hos-text-muted text-lg">No revenue data for this period</p>
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-hos-border">
                      {periods.map((row: any, idx: number) => {
                        const revenue = row.totalRevenue || row.revenue || 0;
                        const orders = row.orderCount || row.orders || 0;
