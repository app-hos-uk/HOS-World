...
export default function WholesalerBulkProductsPage() {
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `wholesale-products-export-${new Date().toISOString().split('T')[0]}.csv`;
    ...
    <RouteGuard allowedRoles={['WHOLESALER', 'ADMIN']} showAccessDenied={true}>
      <DashboardLayout role="WHOLESALER" menuItems={menuItems} title="Wholesaler">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Import or export wholesale products in bulk using CSV files</p>
        </div>
