...
function formatTimestamp(dateStr: string): string {
  ...
  return formatDateLib(date, { day: 'numeric', month: 'short', year: 'numeric' });
}
    ...
    <RouteGuard allowedRoles={[...ALL_ROLES]} showAccessDenied={true}>
      <div className="min-h-screen bg-hos-bg-secondary">
        ...
        <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
            ...
            {unreadCount > 0 && (
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 bg-hos-gold text-[#1a1406] rounded-lg hover:bg-hos-gold-hover transition-colors font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {actionLoading === 'all' ? (
          ...
          {/* Filter Tabs */}
          <div className="flex gap-1 mb-6 bg-hos-bg-secondary rounded-lg shadow p-1 w-fit">
            <button
          ...
          {loading ? (
            ...
            <div className="bg-hos-bg-secondary rounded-lg shadow p-8 text-center">
              ...
              <p className="text-hos-text-secondary mb-4">{error}</p>
              <button
            ...
            <div className="space-y-3">
              {filteredNotifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`bg-hos-bg-secondary rounded-lg shadow overflow-hidden transition-all ${
                    !notification.readAt
                      ? 'border-l-4 border-l-hos-gold'
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-hos-text-muted mt-1 line-clamp-2">
                        ...
                        {/* Unread dot (desktop) */}
                        {!notification.readAt && (
                          <span className="hidden sm:block w-2.5 h-2.5 bg-hos-gold rounded-full flex-shrink-0 mt-1.5" />
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-3">
                        {!notification.readAt && (
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-hos-gold-hover bg-hos-gold/10 rounded-md hover:bg-hos-gold/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {actionLoading === notification.id ? (
                              <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-hos-gold" />
                            ) : (
