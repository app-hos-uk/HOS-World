...
export default function SellerProductsPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await apiClient.getSellerProducts();
      if (response?.data) {
  ...
  const stats = useMemo(() => {
    ...
    const pending = products.filter(p => ['PENDING', 'PENDING_REVIEW', 'DRAFT'].includes(p.status)).length;
    ...
    return { total: products.length, active, inactive, pending, outOfStock, lowStock, totalValue };
  }, [products]);
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredProducts.map((product) => (
                        <tr key={product.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(product.status, product.stock || 0)}`}>
                              {getDisplayStatus(product.status, product.stock || 0)}
                            </span>
                          </td>
