...
/** Valid https links only — avoids `#` stubs that confuse guests */
function sanitizeSocialHref(url: string | undefined): string | null {
  if (!url || typeof url !== 'string') return null;
  const t = url.trim();
  return /^https?:\/\/.+/i.test(t) ? t : null;
}

type SocialEntry = { label: string; ariaLabel: string; href: string };
...
export function Footer() {
  ...
  const socialEntries = useMemo(() => {
    ...
    const fb = sanitizeSocialHref(process.env.NEXT_PUBLIC_SOCIAL_FACEBOOK_URL);
    if (fb) out.push({ label: 'Facebook', ariaLabel: 'Facebook', href: fb });
    const x = sanitizeSocialHref(process.env.NEXT_PUBLIC_SOCIAL_X_URL ?? process.env.NEXT_PUBLIC_SOCIAL_TWITTER_URL);
    if (x) out.push({ label: 'X (Twitter)', ariaLabel: 'X (Twitter)', href: x });
    const ig = sanitizeSocialHref(process.env.NEXT_PUBLIC_SOCIAL_INSTAGRAM_URL);
    if (ig) out.push({ label: 'Instagram', ariaLabel: 'Instagram', href: ig });
    return out;
  }, []);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-6 sm:mb-8">
          ...
          <div>
            ...
            {socialEntries.length > 0 ? (
              <div className="flex flex-wrap gap-3 sm:gap-4" style={{ color: theme.colors.text.secondary }}>
                {socialEntries.map(({ label, ariaLabel, href }) => (
                  <a
                    key={label}
                    href={href}
                    target="_blank"
