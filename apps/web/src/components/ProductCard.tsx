...
export function ProductCard({
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    if (demo) {
      toast.success('Demo item — connect products API to add to basket');
      return;
    }
    if (adding) return;
    setAdding(true);
    try {
      ...
      trackAddToCart({ id, name, price, currency: 'USD' }, 1);
  ...
  const CardInner = (
    <div className="product-card-ref group flex flex-col h-full w-full">
      ...
      <div className="p-3.5 flex flex-col flex-1 gap-1.5">
        <h3 className="font-display text-[0.98rem] text-hos-text-primary leading-snug line-clamp-2">
          {name}
        </h3>
        ...
        <div className="flex items-baseline gap-2.5 flex-wrap mt-auto pt-1">
          <span className="text-hos-gold font-ui font-semibold text-sm">
            {formatPrice(price)}
          </span>
          {hasDiscount && (
  ...
  if (demo) {
    return <div className="flex h-full">{CardInner}</div>;
  }

  return (
    <Link href={`/products/${id}`} className="group flex h-full">
      {CardInner}
    </Link>
  );
}
