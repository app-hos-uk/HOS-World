...
export function ProductCard({
  ...
  const imageContent = (hoverable: boolean) => (
    <>
      {imageUrl ? (
        <SafeImage
          src={imageUrl}
          ...
          className={`object-contain object-center p-3${hoverable ? ' group-hover:scale-105 transition-transform duration-300' : ''}`}
          fallback="📷"
        />
      ) : (
        <div className="flex w-full h-full items-center justify-center text-hos-text-muted">
  ...
  const ImageBlock = productHref ? (
    <Link href={productHref} className="relative aspect-square overflow-hidden bg-hos-bg-tertiary border-b border-hos-gold-15 block">
      {imageContent(true)}
      {vendor && (
    ...
    <div className="relative aspect-square overflow-hidden bg-hos-bg-tertiary border-b border-hos-gold-15">
      {imageContent(false)}
    </div>
  );
